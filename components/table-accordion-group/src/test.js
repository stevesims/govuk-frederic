...
describe('TableAccordionGroup', () => {
  ...
  it('triggers onChange when OpenButton is clicked', () => {
    ...
    const wrapper = mount(<TableAccordionGroup expanded="Expanded" onChange={changeHandler}>
        Test
    </TableAccordionGroup>);
  ...
  it('triggers onChange when title is clicked and changeOnTitleClick is true', () => {
    const changeHandler = jest.fn();
    const wrapper = mount(<TableAccordionGroup expanded="Expanded" onChange={changeHandler} changeOnTitleClick>
        Test
    </TableAccordionGroup>);
    wrapper.find('Title').simulate('click');
    expect(changeHandler).toHaveBeenCalledTimes(1);
