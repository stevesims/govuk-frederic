export const objectHasValueForKeys =
  (item = {}, keys = []) =>
    Object.entries(item).reduce((hasValue, [key, value]) =>
      hasValue || (keys.includes(key) && (value !== undefined)), false);

export const arrayHasValueForKeys =
  (array = [], keys = []) =>
    ((array.length > 0) && array.reduce((hasValue, item) =>
      hasValue || objectHasValueForKeys(item, keys), false));
...
export const rowsFromArray = (array, fields, skipEmptyRows, defaultTransform = value => (value || '-')) => {
  ...
  return array.reduce((rows, item) => {
    if (!skipEmptyRows || objectHasValueForKeys(item, keys)) {
      rows.push(fields.map(({ key, transform = defaultTransform }) => transform(item[key])));
    }
