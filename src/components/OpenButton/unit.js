...
test(
  'OpenButton: Component renders when open',
  assert => {
...
test(
  'OpenButton: Component onChange event is called when clicked',
  assert => {
    ...
    assert.equal(
      handleClick.getCall(0).args[0].open, true // eslint-disable-line security/detect-non-literal-fs-filename
    );
    assert.end();
  }
);
