...
const TableContainer = glamorous('table', {
  ...
  { tableLayout: flexibleColumns ? 'auto' : 'fixed' }
));
      ...
      <thead>
        <tr>
          {titles.map((title, index) =>
            ...
            <TableHeading key={title.key || index} name={names[index]}>{title}</TableHeading>
          )}
        </tr>
      </thead>
    }
        ...
        <tr key={index}>
          {row.map((item, itemIndex) =>
            (rowIncludesHeading && (itemIndex === 0) ?
              ...
              // disable false-positive rule - this is an access into an array of strings, not object access
              // eslint-disable-next-line security/detect-object-injection
              <TableData key={item.key || itemIndex} name={names[itemIndex]}>{item}</TableData>
            )
          )}
        </tr>
      )}
